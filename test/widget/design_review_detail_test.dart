...
void main() {
  ...
  Widget createTestWidget(String reviewId) {
    return ProviderScope(
      ...
      child: MaterialApp(
        home: DesignReviewDetailScreen(reviewId: reviewId),
      ),
    );
  }

  testWidgets('DesignReviewDetailScreen shows stages and info', (WidgetTester tester) async {
    ...
    when(() => mockRepository.watchReviews()).thenAnswer((_) => Stream.value([review]));
    when(() => mockRepository.getAllReviews()).thenAnswer((_) => Future.value([review]));
  ...
  testWidgets('Adding a stakeholder calls repository', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1200, 1200);
    addTearDown(() => tester.view.resetPhysicalSize());
    ...
    when(() => mockRepository.watchReviews()).thenAnswer((_) => Stream.value([review]));
    when(() => mockRepository.getAllReviews()).thenAnswer((_) => Future.value([review]));
    when(() => mockRepository.saveReview(any())).thenAnswer((_) => Future.value());
  ...
  testWidgets('Updating substep status updates progress', (WidgetTester tester) async {
    tester.view.physicalSize = const Size(1200, 1200);
    addTearDown(() => tester.view.resetPhysicalSize());
    ...
    final review = DesignReview(
      ...
      stages: [
        Stage(
          ...
          lastUpdated: DateTime.now(),
          subSteps: [
            SubStep(id: 'ss-1', name: 'Task 1', workspaceId: 'ws-1', status: StageStatus.notStarted),
          ],
        ),
      ],
    );
